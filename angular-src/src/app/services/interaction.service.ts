...
export class InteractionService {
  ...
  registerWork(task_tag:string, work_completed:number) {
    ...
    return this.http.post<any>('users/registerwork', task_info, { headers:headers });
  }
  ...
  registerChoice(allocation_tag:string, choices:any) {
    ...
    this.loadToken();
    let headers = new HttpHeaders().append('Content-Type','application/json').append('Authorization',this.authToken)
    return this.http.post<any>('users/registerChoice', choice_info, { headers:headers });
  }
  registerAllocationText(allocation_tag:string, text) {
    let choice_info = { tag: allocation_tag, text:text }
    this.loadToken();
    let headers = new HttpHeaders().append('Content-Type', 'application/json').append('Authorization', this.authToken);
    return this.http.post<any>('users/registerAllocationText', choice_info, { headers:headers});
  }

  registerRegChoice(submit:any) {
    this.loadToken();
    let headers = new HttpHeaders().append('Content-Type','application/json').append('Authorization',this.authToken)
    return this.http.post<any>('users/registerregchoice', submit, { headers:headers });
  }

  setAllocation(user:any) {
    this.loadToken()
    let headers = new HttpHeaders().append('Content-Type','application/json').append('Authorization',this.authToken)
    return this.http.post<any>('users/setallocation', user, { headers:headers });
  }
  ...
  getSuggestion(user:any, week:number) {
    ...
    let info = { user, week}
    let headers = new HttpHeaders().append('Content-Type','application/json').append('Authorization',this.authToken)
    return this.http.post<any>('users/getsuggestion', info, { headers:headers });
  }
  timestamp(user:any, time:string, task:string, part:string) {
    this.loadToken()
    let info = {user, time, task, part};
    let headers = new HttpHeaders().append('Content-Type','application/json').append('Authorization',this.authToken)
    return this.http.post<any>('users/timestamp', info, { headers:headers })
  }
  instructionsReport(user:any, report:any) {
    this.loadToken()
    let headers = new HttpHeaders().append('Content-Type','application/json').append('Authorization',this.authToken)
    return this.http.post<any>('users/instructionsreport', {user, report}, { headers:headers })
  }
}
